      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          <div key={m.label} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
            <p className="text-zinc-400 text-[10px] sm:text-xs uppercase tracking-wider mb-1">{m.label}</p>
            <p className={`text-xs sm:text-sm xl:text-xl font-bold ${m.color}`}>{m.value}</p>
          </div>
        ))}
      </div>
